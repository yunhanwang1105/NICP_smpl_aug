...
bm_fname = "/mnt/qb/work/ponsmoll/pba594/body_models/smplh/neutral/model.npz"
...
class AMASSDataset(Dataset):
    def __init__(
        ...
        **kwargs
        ...
        # We take the length from num data files
        directory = self.path + str(mode) + "/ifnet_indi" + "/verts_occ_dist/"
        self.len = len([f for f in os.listdir(directory) if os.path.isfile(os.path.join(directory, f))])
    ...
    def __getitem__(self, idx):
        ...
        data = {}
        # Take the vertices
        data["data_v_scal_noise"] = torch.load(os.path.join(self.path, str(self.mode), "ifnet_indi", "verts_" + str(self.type) , str(f'{int(idx):09}')) + ".pt")
        data["data_v_smpl"] = torch.load(os.path.join(self.path, str(self.mode), "ifnet_indi", "smpl_verts", str(f'{int(idx):09}')) + ".pt")
        ...
        # Sample random points near the SMPL surface (maybe noised, clothed)
        smpl_inds = np.arange(data["data_v_scal_noise"].shape[0])
        np.random.shuffle(smpl_inds)
        smpl_inds = smpl_inds[:self.n_fine_sampled]
        noise_smpl = np.random.normal(0, self.fine_std, (self.n_fine_sampled, 3))
        rand_smpl =  data["data_v_scal_noise"][smpl_inds] + noise_smpl

        point_pos = np.concatenate((rand_uniform, rand_smpl))
        _target_smpl = data["data_v_smpl"][self.idxs]
        ...
        sample = {
                  ...
                  'smpl_vertices': torch.as_tensor(_target_smpl,dtype=torch.float32),
                  'idxs': self.idxs,
                  'smpl_vertices_full': torch.as_tensor(data["data_v_smpl"],dtype=torch.float32),
                  'smpl_inds': smpl_inds,
                  'smpl_inds_wgt': smpl_inds_wgt,
                  'local_smpl': torch.as_tensor(local_smpl,dtype=torch.float32),
                  'labels': self.labels
