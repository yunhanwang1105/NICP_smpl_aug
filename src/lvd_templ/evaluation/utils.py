...
def SMPL_fitting(SMPL_model, in_points, gt_idxs, prior, iterations = 1000):
    ...
    with torch.no_grad():
        pose, beta, trans, scale = parameters_smpl.forward()
        forward_verts, forward_joint_pos, forward_Rs = SMPL_model.forward(theta=pose, beta=beta, get_skin=True)
        vertices_smpl = (forward_verts[0] + trans)*scale
        ...
        params['trans'] = trans
        params['scale'] = scale
        params['fit_joint'] = forward_joint_pos[0]
...
def vox_scan_smpl_aug(voxel_src, res, style='occ', grad=0,device='cuda', margin=0.8,center=True,scale=True):
